...
void ofApp::update() {
	...
	for (int i = 0; i < 15; i++) {
		...
		for (int k = 0; k < 4; k++) {
			...
			auto location = glm::vec2(radius * cos(i * 24 * DEG_TO_RAD), radius * sin(i * 24 * DEG_TO_RAD));
			auto next = glm::vec2((radius - 10) * cos(i * 24 * DEG_TO_RAD), (radius - 10) * sin(i * 24 * DEG_TO_RAD));
